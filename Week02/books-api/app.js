...
app.post("/books", (req, res) => {
    const newBook = req.body; // Get the new book from the request body
...
app.get("/books/:id", (req, res) => {
    const bookId = parseInt(req.params.id); // Get book from URL parameter
...
app.delete("/books/:id", (req, res) => {
    ...
    if (bookIndex !== -1) {
        ...
        res.status(404).status.send("Book not found"); // Send error for non-existent book
    }
});
